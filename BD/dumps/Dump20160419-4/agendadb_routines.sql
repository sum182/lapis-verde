...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `sp_tabela_atualizacao_set`(p_tabela varchar(400), p_escola_id int)
BEGIN
 ...
 call sp_set_time_zone;
  ...
  select count(*)
  into qtde
  from tabela_atualizacao t
  where t.tabela = p_tabela
  and t.escola_id = p_escola_id;
  
  if qtde > 0 then
	  update tabela_atualizacao t
	  set t.data = now()
	  where t.tabela = p_tabela
      and t.escola_id = p_escola_id;
      
  else
    insert into tabela_atualizacao(tabela,data,escola_id) values(p_tabela,now(),p_escola_id);
  end if;
